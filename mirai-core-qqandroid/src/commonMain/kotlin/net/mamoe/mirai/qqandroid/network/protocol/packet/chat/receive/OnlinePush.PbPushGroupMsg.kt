...
import net.mamoe.mirai.qqandroid.FPM
...
internal object OnlinePushPbPushGroupMsg : IncomingPacketFactory<Packet?>("OnlinePush.PbPushGroupMsg") {
    ...
    override suspend fun ByteReadPacket.decode(bot: QQAndroidBot, sequenceId: Int): Packet? {
        ...
        var message = pbPushMsg.msg.toMessageChain(bot, groupIdOrZero = group.id, onlineSource = true)
        val flags = extraInfo?.flags ?: 0
        if ((pbPushMsg.msg.contentHead?.pkgNum ?: 0) > 1) {
            val ch = pbPushMsg.msg.contentHead
            if (ch != null) {
                val id = ch.divSeq
                val fpm = FPMM.getFpmOrNew(id, ch.pkgNum)
                val rm = fpm.putMessage(ch.pkgIndex, message)
                if (rm != null) {
                    message = rm
                    FPMM.removeFpm(id)
                } else return null
